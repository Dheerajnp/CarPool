import { MapPinIcon, CalendarIcon, UserIcon, CarIcon, ArrowRightIcon } from 'lucide-react'
...
export default function RideDetailsPage() {
  ...
  useEffect(() => {
    const fetchRideDetails = async () => {
        setIsLoading(true)
      try {
        const response = await axiosApiGateway.get(`/ride/getRideDetails/${rideId}`)
  ...
  const getRideStatusColor = (status: any) => {
    switch (status) {
      case 'pending':
      ...
      <Card className="w-full max-w-3xl mx-auto">
        <CardHeader>
          <div className="flex justify-between items-center">
            ...
            <Badge className={`text-lg px-3 py-1 ${getRideStatusColor(rideData.status)}`}>
              {rideData.status}
            </Badge>
          </div>
        </CardHeader>
          ...
          <div className="grid gap-6">
            ...
            <div className="grid gap-2">
              ...
              <div className="bg-muted p-4 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium">{rideData.vehicle.brand} {rideData.vehicle.model}</span>
                </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <Avatar>
                  <AvatarImage src={rideData.driver.profile} alt={rideData.driver.name} />
                  <AvatarFallback>{rideData.driver.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
                </Avatar>
                <div>
                  <p className="font-medium">{rideData.driver.name}</p>
                  <p className="text-sm text-muted-foreground">{rideData.driver.email}</p>
                </div>
              </div>
            </div>
            <div>
                ...
                {rideData.passengers.map((passenger, index) => (
                  <li key={index} className="flex items-center gap-2">
                    <Avatar>
                      ...
                      <AvatarFallback>{passenger.rider.name.split(' ').map(n => n[0]).join('')}</AvatarFallback>
                    </Avatar>
                    <span>{passenger.rider.name} ({passenger.numberOfPassengers} seat(s))</span>
                  </li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <Badge variant="secondary" className="text-lg"> ₹ {rideData.price} per seat</Badge>
            </div>
          </div>
        </CardContent>
        <CardFooter className="flex justify-between">
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              ...
              <DialogFooter>
                <Button variant="outline" onClick={() => setIsDialogOpen(false)}>No, keep the ride</Button>
                <Button variant="destructive" onClick={handleCancelRide}>Yes, cancel the ride</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          <Button onClick={()=>handleChatClick(rideData?.driver._id as string)}>Contact Driver</Button>
        </CardFooter>
      </Card>
    </div>
